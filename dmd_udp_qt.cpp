...
void UDPPayload::toByteArray(QByteArray data)
{
  data[0] = tot_size >> 8;
  data[1] = tot_size & 0xFF;
  data[2] = rec_id >> 8;
  data[3] = rec_id & 0xFF;

  if (payload) payload->toByteArray(data);
}
/*-----------------------------------------------------------------------------------------------*/
...
{
  memcpy(buf,&unsigned_short_int,2);
}
...
{
  memcpy(buf,&unsigned_long_int,4);
}
...
{
  memcpy(buf,&pkg_no,2);
  memcpy(&buf[2],&tot_pkg,2);
  memcpy(&buf[4],&data_size,2);
...
{
  memcpy(buf,&in_puls_e,4);
  memcpy(&buf[4],&in_puls_o,4);
  memcpy(&buf[8],&min_per_e,2);
  memcpy(&buf[10],&min_per_o,2);
  memcpy(&buf[12],&seq_puls,2);
}
...
void NetworkSettings::toBytes(char* buf)
{
  ...
  memcpy(&buf[4],&subnet,4);
  memcpy(&buf[8],&gateway,4);
...
{
  memcpy(buf,&sys_fw_major,2);
  memcpy(&buf[2],&sys_fw_minor,2);
  memcpy(&buf[4],&app_sw_ver,2);
  memcpy(&buf[6],&app_sw_year,2);
...
{
  memcpy(buf,&dmd_type,2);
  memcpy(&buf[2],&blocks,2);
  memcpy(&buf[4],&cols,2);
  memcpy(&buf[6],&rows,2);
  memcpy(&buf[8],&rows_pr_block,2);
}
