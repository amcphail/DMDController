...
Payload* DMD::sendMessage(UDPPayload* message)
{
  ...
  message->toByteArray(send_data);

  socket->write(send_data);

  recv_data = socket->read(BUFLEN);

  response = new UDPPayload(recv_data.data());
...
Payload* DMD::sendCompoundMessage(int num, UDPPayload** messages)
{
  ...
  for (i = 0; i < num; i++) {
    ...
    send_data.clear();
    messages[i]->toByteArray(send_data);
